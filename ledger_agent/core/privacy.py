"""
ledger_agent/core/privacy.py  –  Enterprise PII / sensitive-data egress firewall (R-46)
────────────────────────────────────────────────────────────────────────────────────────
ARCH-20: Canonical location. The legacy path ``core.privacy`` is a
...
# ── Corpus single-token names (BUG-P2, loaded once) ──────────────────────────

_CORPUS_SINGLE_NAMES: Optional[Set[str]] = None
...
def _load_corpus_single_names() -> Set[str]:
    """
    BUG-P2 fix: load single-word real names from config/redaction_corpus.yaml.
    Returns a set of names (original casing) that should be detected even as
    bare single tokens — the ≥2-token gate of _ALLCAPS_SEQ_PAT misses these.
    """
    global _CORPUS_SINGLE_NAMES
    if _CORPUS_SINGLE_NAMES is not None:
        return _CORPUS_SINGLE_NAMES
    names: Set[str] = set()
    corpus_path = (
        Path(__file__).resolve().parent.parent.parent / "config" / "redaction_corpus.yaml"
    )
    if corpus_path.exists():
        try:
            import yaml  # optional; gracefully absent if pyyaml not installed
            data = yaml.safe_load(corpus_path.read_text(encoding="utf-8")) or {}
            for section in ("entities", "partners", "banks", "brokers", "tickers"):
                for entry in (data.get(section) or []):
                    real = (entry.get("real") or "") if isinstance(entry, dict) else ""
                    stripped = real.strip()
                    # Only single-word entries — multi-word names already caught by
                    # _ALLCAPS_SEQ_PAT (which requires ≥2 all-caps tokens).
                    if stripped and " " not in stripped and stripped.upper() not in _SYSTEM_WORDS:
                        names.add(stripped)
        except Exception:
            pass
    _CORPUS_SINGLE_NAMES = names
    return names
...
_API_KEY_PATTERNS: List[re.Pattern] = [
    ...
    re.compile(r"\b[a-f0-9]{32,64}\b(?=[^\w]|$)(?=.{0,200}\b(?:secret|token|key|password)\b)",
               re.IGNORECASE),  # hex secrets with hint word (SMELL-P6: bounded lookahead)
]
...
# BUG-P3: require EIN/TIN keyword context within ±40 chars to avoid false
# positives on invoice IDs, batch IDs, and fiscal-year prefixes (e.g. 2024-1234567).
_EIN_CTX = re.compile(
    r"\b(?:EIN|TIN|TAX[\s\-]?ID|EMPLOYER[\s\-]?(?:IDENTIFICATION[\s\-]?)?(?:NUMBER|ID))\b",
    re.IGNORECASE,
)
...
# BUG-P1: context gate for bare NANP phone numbers (no parentheses or
# explicit +1 prefix). Without a context keyword nearby, a 10-digit run
# (e.g., account/invoice number) matches the NANP pattern and produces
# false-positive redactions in ledger data.
# Formatted variants with parentheses or +1 prefix are self-identifying and
# fire unconditionally; only the ambiguous plain-digit form requires context.
_PHONE_CTX = re.compile(
    r"\b(?:PHONE|TEL(?:EPHONE)?|MOBILE|CELL(?:ULAR)?|FAX|"
    r"CONTACT|CALL|SMS|HOTLINE|DIRECT|EXT(?:ENSION)?|"
    r"REACH|OFFICE|HOME\s+(?:PHONE|NUMBER))\b",
    re.IGNORECASE,
)
...
def _detect_ein(text: str) -> List[Tuple[str, str]]:
    ...
    for m in _EIN_PAT.finditer(text):
        # BUG-P3: only flag NN-NNNNNNN as an EIN when a context keyword
        # (EIN, TIN, TAX-ID, EMPLOYER-ID) appears within ±40 characters.
        # Without context, invoice IDs and fiscal-year prefixes match the same
        # pattern and produce false positives.
        start = max(0, m.start() - 40)
        end = min(len(text), m.end() + 40)
        window = text[start:end]
        if _EIN_CTX.search(window):
...
def _detect_phones(text: str) -> List[Tuple[str, str]]:
    ...
    for m in _PHONE_PAT.finditer(text):
        ...
        if len(digits) < 10:
            continue
        # BUG-P1: self-identifying formats (parenthesised area code or +1 prefix)
        # fire unconditionally. Ambiguous plain-digit sequences require a phone
        # context keyword within ±60 chars to avoid false positives on ledger
        # numbers (invoice IDs, account numbers, EINs, etc.).
        stripped = orig.strip()
        is_formatted = (
            stripped.startswith("+1")
            or stripped.startswith("(")
            or (len(stripped) > 10 and not stripped[:3].isdigit())
        )
        if not is_formatted:
            start = max(0, m.start() - 60)
            end = min(len(text), m.end() + 60)
            if not _PHONE_CTX.search(text[start:end]):
                continue
...
def _detect_corpus_names(text: str) -> List[Tuple[str, str]]:
    """
    BUG-P2 fix: detect corpus-resident single-token entity/partner/bank/broker
    names that slip past _ALLCAPS_SEQ_PAT (which requires ≥2 all-caps tokens).

    Deliberately does NOT check the vendor allowlist — corpus names are
    explicitly identified PII for this partnership and take priority over
    general vendor suppression.  _SYSTEM_WORDS exclusion still applies to
    prevent redacting generic protocol acronyms.

    Produces COUNTERPARTY tokens (same category as _detect_counterparty_names).
    """
    names = _load_corpus_single_names()
    if not names:
        return []
    hits: List[Tuple[str, str]] = []
    seen: Set[str] = set()
    for name in names:
        ...
        for m in pat.finditer(text):
            ...
            if key in seen:
                continue
            seen.add(key)
            token = _stable_token("COUNTERPARTY", key)
            hits.append((orig, token))
    return hits
...
_ALL_DETECTORS = [
    ...
    "corpus_names",  # BUG-P2: single-token corpus-resident entities
]
...
_SCOPE_DETECTORS: Dict[str, List[str]] = {
    ...
    "log": [
        ...
        "driver_license", "passport", "dob",
        "ipv4", "ipv6", "mac", "url_creds",
        "entity_name", "person_name", "counterparty", "corpus_names",
    ],
}
...
def redact(
    ...
    if "corpus_names" in detectors:
        replacements.extend(_detect_corpus_names(text))
