"""
ledger_agent/core/models.py  –  Pure-Python dataclass models (no DB coupling)
─────────────────────────────────────────────────────────────────────────────
All monetary values are stored as Python Decimal for exactness.
All IDs are UUID4 strings generated at construction time.

ARCH-20: Canonical location. The legacy path ``core.models`` is a
...
class TransactionType(str, Enum):
    ...
    DIVIDEND = "dividend"
    OTHER = "other"
    PRIOR_PERIOD_ADJUSTMENT = "prior_period_adjustment"  # ARCH-31: opening balance continuity
