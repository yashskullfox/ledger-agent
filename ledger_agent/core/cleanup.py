"""
ledger_agent/core/cleanup.py  –  Raw-data cleanup cycle (R-46 / Section 7)
────────────────────────────────────────────────────────────────────────────────
ARCH-20: Canonical location. The legacy path ``core.cleanup`` is a
...
Public API
----------
    from core.cleanup import run_cycle, register_cleanup_hook, boot_cleanup
...
# ARCH-20: File is now at ledger_agent/core/cleanup.py so parent.parent.parent
...
# BUG-C1 fix: per-cycle write lock shared between active writers and cycle_cleanup.
# Writers acquire this lock (via hold_write_lock()) before writing to raw_cache or
# export_tmp.  cycle_cleanup tries to acquire with a short timeout; if it cannot
# (a writer is active) it emits cleanup.skipped_busy and defers cleanup to the
# next cycle rather than racing.  Using RLock so cleanup-from-within-a-worker is
# a no-op (same thread can re-acquire without deadlock).
_write_lock = threading.RLock()
_WRITE_LOCK_TIMEOUT_SECS: float = 2.0
...
def hold_write_lock() -> Iterator[None]:
    """
    BUG-C1: Context manager for code that writes transient files to raw_cache
    or export_tmp.  Acquire before writing so cycle_cleanup knows to back off.

    Usage::

        with hold_write_lock():
            (tmp_path / "data.json").write_text(json_str)
    """
    _write_lock.acquire()
    try:
        yield
...
def _purge_process_scratch() -> int:
    ...
    SMELL-C4 fix: only remove entries owned by the current effective user.
    On a shared host another user could pre-create a ``ledger-agent-raw-*``
    path; deleting it without an ownership check is a confused-deputy error.
    Skipped entries are logged and (if audit is available) emit
    ``cleanup.skipped_foreign_uid`` so the operator knows the scratch dir was
    not fully cleaned.
    """
    my_uid: Optional[int] = getattr(os, "getuid", lambda: None)()
    tmp = Path(tempfile.gettempdir())
    removed = 0
    for entry in tmp.glob(f"{_PROCESS_SCRATCH_PREFIX}*"):
        try:
            if my_uid is not None:
                entry_uid = entry.stat().st_uid
                if entry_uid != my_uid:
                    _log.debug(
                        "cleanup: skipping foreign-uid scratch: %s (uid=%d)",
                        entry.name, entry_uid,
                    )
                    try:
                        from ledger_agent.core.audit import audit
                        audit("cleanup.skipped_foreign_uid",
                              path=entry.name, entry_uid=entry_uid)
                    except Exception:
                        pass
                    continue
            if entry.is_dir():
...
def cycle_cleanup(label: str = "cycle") -> None:
    ...
    BUG-C1 fix: acquires ``_write_lock`` with a short timeout before touching
    the file system.  If a writer is currently active (holding the lock via
    ``hold_write_lock()``) the cleanup is skipped and a ``cleanup.skipped_busy``
    audit event is emitted rather than racing against the active write.
    """
    try:
        from ledger_agent.core.audit import audit
    except Exception:
    ...
    acquired = _write_lock.acquire(timeout=_WRITE_LOCK_TIMEOUT_SECS)
    if not acquired:
        _log.warning("cleanup: skipped %r (write lock held by active writer)", label)
        if audit:
            audit("cleanup.skipped_busy", label=label)
        return
    try:
        raw = _purge_directory(_RAW_CACHE_DIR)
        exp = _purge_directory(_EXPORT_TMP_DIR)
        scratch = _purge_process_scratch()
        hooks_ran = _run_hooks()
        _purge_privacy_session()
