"""
ledger_agent/core/database.py  –  SQLite persistence layer (Repository pattern)
─────────────────────────────────────────────────────────────────────────────────
ARCH-20: Canonical location. The legacy path ``core.database`` is a
...
SCHEMA_VERSION = 4  # ARCH-27: added classification_lock_at to transactions
...
def init_db(db_path: Optional[Path] = None) -> None:
    ...
    with get_conn(db_path) as conn:
        ...
        if current < SCHEMA_VERSION:
            # ARCH-27 (v3→v4): persist classification timestamp
            if current < 4:
                try:
                    conn.execute(
                    ...
                    pass  # column already exists on a newer schema
            conn.execute(
...
_DEFAULT_COA: list[tuple] = [
    ...
    # ── Assets ──────────────────────────────────────────────────────────────
    ("1000", "Cash & Cash Equivalents", "asset", None, "", '["cash","checking","balance"]'),
    ...
    ("2020", "Taxes Payable", "liability", "2000", "", '["taxes payable","tax accrual"]'),
    ...
    ("3040", "Members Distributions / Owner Draws", "equity", "3000", "",
     '["usataxpymt","estimated tax","owner draw","distribution","member draw"]'),
    # ── Revenue ─────────────────────────────────────────────────────────────
    ("4000", "Revenue", "revenue", None, "", '[]'),
    ("4010", "Realised Trading Gains", "revenue", "4000", "", '["gain","sold","proceeds","realized gain"]'),
    ("4020", "Service Revenue", "revenue", "4000", "", '["intuit","deposit","invoice","counterdeposit"]'),
    ...
    ("5050", "Federal Income Tax Expense", "expense", "5000", "", '["federal income tax expense"]'),
...
class TransactionRepo:
    ...
    def update_coa(tx_id: str, coa_code: str, coa_name: str,
        ...
        """Persist COA classification and stamp classification_lock_at (ARCH-27)."""
        with get_conn(db_path) as conn:
            conn.execute(
                "UPDATE transactions SET coa_code=?, coa_name=?,"
                " classification_lock_at=? WHERE id=?",
                (coa_code, coa_name,
                 datetime.now(timezone.utc).isoformat(), tx_id),
            )
...
class ImportRegistry:
    """Legacy import-tracking repo keyed on (account_id, period).

    Prefer ``ImportedStatementRepo`` for new code — it uses file-path dedup
    which is cheaper and avoids the account-id look-up requirement.
    """
...
class ImportedStatementRepo:
    """File-path-based import tracking used by ``ledger_agent.core.api``.

    Checks and records by ``source_file`` (PDF path) + ``account_id`` so that
    re-importing the same file is a cheap O(1) skip without a secondary query.
    """

    @staticmethod
    def exists(file_path: str, db_path: Optional[Path] = None) -> bool:
        """Return True if this PDF has already been imported."""
        with get_conn(db_path) as conn:
            row = conn.execute(
                "SELECT id FROM imported_statements WHERE source_file=? LIMIT 1",
                (file_path,),
            ).fetchone()
        return row is not None
    ...
    def record(
        ...
        """Record a successfully imported statement. Idempotent (OR IGNORE)."""
        import uuid as _uuid
        with get_conn(db_path) as conn:
            conn.execute(
                "INSERT OR IGNORE INTO imported_statements"
                "(id,source_file,parser_id,account_id,statement_period,imported_at)"
                " VALUES(?,?,?,?,?,?)",
                (str(_uuid.uuid4()), file_path, parser_id, account_id,
                 period, datetime.now(timezone.utc).isoformat()),
            )
